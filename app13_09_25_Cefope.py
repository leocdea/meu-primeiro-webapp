...
if df_original is not None:
    ...
    # Filtros com tratamento de valores nulos (para evitar erros)
    ...
    # A linha abaixo era a provável causa do erro. Esta versão está corrigida.
    escolaridade_options = sorted(df_original['Escolaridade'].dropna().unique())
    escolaridade_selecionada = st.sidebar.multiselect(
        'Escolaridade:', options=escolaridade_options, default=escolaridade_options
    )

    cargo_options = sorted(df_original['Cargo'].dropna().unique())
    cargo_selecionado = st.sidebar.multiselect(
        'Cargo:', options=cargo_options, default=cargo_options
    )
